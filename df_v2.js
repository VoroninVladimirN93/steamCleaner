...
// Завершение процесса Steam
function killSteam() {
    console.log('Закрытие клиента Steam...');
    try {
        execSync('taskkill /IM steam.exe /F');
    } catch (err) {
        console.error('Ошибка при закрытии Steam:', err);
    }
}

// Ожидание завершения процесса Steam
...
function startSteam() {
    console.log('Запуск клиента Steam...');
...
async function main() {
    if (isSteamRunning()) {
        killSteam();
        console.log('Ожидание завершения Steam...');
        await waitForSteamToClose();
    } else {
        console.log('Steam уже закрыт.');
    }

    console.log('Начинается удаление папок...');
    deleteFolders();
